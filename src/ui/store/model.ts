...
export interface Folder {
  ...
  parentFolderId?: string; sortOrder: number;
}

export interface RequestRow {
  id: string; collectionId: string; folderId?: string;
  name: string; method: string; url: string;
  headers: { key: string; value: string }[];
  bodyText: string; bodyKind: string;
...
export interface OpenTab {
  ...
  sortOrder: number; isPinned: boolean; isDirty: boolean;
