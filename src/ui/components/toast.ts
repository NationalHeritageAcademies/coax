...
//
// Intentionally framework-free (no Angular dependency) so it stays drop-in
// usable from anywhere in the renderer — including imperative helpers like
// prompt.ts that run outside change detection.
...
export function showToast(message: string, kind: ToastKind = 'info', optionsOrDuration: number | ToastOptions = 4000): void {
	const options: ToastOptions = typeof optionsOrDuration === 'number' ? { durationMs: optionsOrDuration } : optionsOrDuration;
